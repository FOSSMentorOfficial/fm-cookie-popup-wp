...
if ( ! class_exists( 'CI_Settings' ) ) :
	...
	class CI_Settings {
		...
		public function callback_file( $args ) {
			...
			$html = sprintf( '<input type="text" class="%1$s-text GDPRCookieMaker-url" id="%2$s[%3$s]" name="%2$s[%3$s]" value="%4$s"/>', $size, $args['section'], $args['id'], $value );
			$html .= '<input type="button" class="button GDPRCookieMaker-browse" value="' . $label . '" />';
			$html .= $this->get_field_description( $args );
		...
		public function callback_image( $args ) {
			...
			$html .= '<input type="button" class="button GDPRCookieMaker-browse" value="' . $label . '" />';
			$html .= $this->get_field_description( $args );
			$html .= '<p class="GDPRCookieMaker-image-preview"><img src=""/></p>';
		...
		public function callback_separator( $args ) {
			...
			$html = '';
			$html .= '<div class="GDPRCookieMaker-settings-separator"></div>';
			echo $html;
		}
		...
		public function plugin_page() {
			...
			<div class="cookie-admin-header">
				<div class="logo">
					<!-- <img src="<?php //echo PLUGIN_URL . '/assets/admin/cookie-logo.svg'; ?>" /> -->
				</div>
				<div class="info-links">
				</div>
			</div>
			<div class="wrap GDPRCookieMaker-admin">
		...
		public function script() {
			...
			<script>
				jQuery(document).ready(function ($) {
					...
					$('.GDPRCookieMaker-browse').on('click', function (event) {
					...
					$('input.GDPRCookieMaker-url')
						.on('change keyup paste input', function () {
							var self = $(this);
							self
								.next()
								.parent()
								.children('.GDPRCookieMaker-image-preview')
								.children('img')
								.attr('src', self.val());
						})
						.change();
			...
			<style type="text/css">
				...
				.GDPRCookieMaker-image-preview img {
					height: auto;
					max-width: 70px;
				}

				.GDPRCookieMaker-settings-separator {
					background: #ccc;
					border: 0;
					color: #ccc;
					height: 1px;
